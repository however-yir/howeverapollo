...
def generate_rule_for_protofile(workdir, protofile, proto_deps, py_proto_deps):
    ...
    if not grpc_found:
        if len(proto_deps) == 0:
            return TEMPLATE_NO_DEPS.format(pb_name=pb_name,
                                           protofile=protofile)
        return TEMPLATE_DEPS.format(pb_name=pb_name,
                                    protofile=protofile,
        ...
        if len(proto_deps) == 0:
            return TEMPLATE_GRPC_NO_DEPS.format(pb_name=pb_name,
                                                py_name_grpc=py_name_grpc,
                                                cc_name_grpc=cc_name_grpc,
                                                protofile=protofile)
        else:
            return TEMPLATE_GRPC_DEPS.format(pb_name=pb_name,
                                             py_name_grpc=py_name_grpc,
                                             cc_name_grpc=cc_name_grpc,
                                             protofile=protofile,
                                             pb_context=proto_deps)
